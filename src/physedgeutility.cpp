...
#define CONVEX_NORMAL_EDGE_RESOLVE 0
...
void _btAdjustInternalEdgeContacts(btManifoldPoint& cp, const btCollisionObjectWrapper* colObj0Wrap,const btCollisionObjectWrapper*, int partId0, int index0, int normalAdjustFlags)
{
	//newNormal;
	...
	if(colObj0Wrap->getCollisionShape()->getShapeType() != TRIANGLE_SHAPE_PROXYTYPE)
	{
		...
		{
			auto *physObj0 = static_cast<PhysObj*>(obj0->userData);
			auto *physObj1 = static_cast<PhysObj*>(obj1->userData);
			if(physObj0 != nullptr && physObj1 != nullptr)
			{
				auto *ent0 = physObj0->GetOwner();
				auto *ent1 = physObj1->GetOwner();
				if(ent0 != nullptr && ent0->IsWorld() && ent1 != nullptr && ent1->IsVehicle())
				{
					auto *game = ent1->GetNetworkState()->GetGameState();

					//auto btSrc = cp.m_positionWorldOnB /PhysEnv::WORLD_SCALE;
					//auto btDest = cp.m_positionWorldOnA /PhysEnv::WORLD_SCALE;
					//auto btDir = btDest -btSrc;
					//Vector3 dir {btDir.x(),btDir.y(),btDir.z()};
					//uvec::normalize(&dir);

					//Vector3 src = ent1->GetPosition() +ent1->GetCollisionCenter();//{btSrc.x(),btSrc.y(),btSrc.z()};
					//auto dest = src +dir *5.f;
					//src -= dir *5.f;

					auto src = ent1->GetPosition() +ent1->GetCollisionCenter();
					auto btDest = cp.m_positionWorldOnB + cp.m_normalWorldOnB * cp.m_distance1;
					btDest /= PhysEnv::WORLD_SCALE;

					Vector3 dst {btDest.x(),btDest.y(),btDest.z()};
					//auto dir = dst -src;
					//uvec::normalize(&dir);
					...
					if(r.hit == true)
					{
						data.SetSource(src -dir *50.f);
						auto n = r.normal;
						data.SetTarget(dst +Vector3(5.f,0.f,0.f) +dir *100.f);
						auto r2 = game->RayCast(data);

						if(game->IsClient() == true)
							game->DrawLine(src -dir *50.f,dst +Vector3(5.f,0.f,0.f) +dir *100.f,Color::Lime,12.f);

						if(r2.hit == true && uvec::cmp(r.normal,r2.normal) == false)
						{
							data.SetTarget(dst -Vector3(5.f,0.f,0.f) +dir *100.f);
							auto r3 = game->RayCast(data);

							if(game->IsClient() == true)
								game->DrawLine(src -dir *50.f,dst -Vector3(5.f,0.f,0.f) +dir *100.f,Color::Aqua,12.f);

							if(r3.hit == true && uvec::cmp(r2.normal,r3.normal) == true)
							{
								n = r3.normal;
								std::cout<<"OVERWROTE BY TRACE #3!"<<std::endl;
							}
						}
						results.push_back(game->RayCast(data));

						data.SetTarget(dst +Vector3(1.f,0.f,0.f));
						results.push_back(game->RayCast(data));

						data.SetTarget(dst +Vector3(-2.f,0.f,0.f));
						results.push_back(game->RayCast(data));
						...
						if(d < 0.9f)
						{
							std::cout<<"Dot Product: "<<d<<" ("<<src.x<<","<<src.y<<","<<src.z<<") ("<<dst.x<<","<<dst.y<<","<<dst.z<<")"<<std::endl;
						}
					}
					else
					{
	...
	// TODO: triangleInfoMapPtr can be invalid in some cases, unsure why (Causes crash!)
