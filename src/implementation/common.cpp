...
static btScalar calculateCombinedRestitution(float restitution0, float restitution1) { return restitution0 * restitution1; }

static bool get_collision_object_properties(pragma::Game *game, const btCollisionObject *o, float *friction, float *restitution)
{
	*friction = 1.f;
	*restitution = 1.f;
	auto *obj = static_cast<pragma::physics::ICollisionObject *>(o->getUserPointer());
	if(obj == nullptr)
		return false;
	auto *mat = game->GetSurfaceMaterial(obj->GetSurfaceMaterial());
	if(mat == nullptr)
		return false;
	*friction = mat->GetDynamicFriction();
...
btBridgedManifoldResultCustom::btBridgedManifoldResultCustom(const btCollisionObjectWrapper *obj0Wrap, const btCollisionObjectWrapper *obj1Wrap, btCollisionWorld::ContactResultCallback &resultCallback) : btManifoldResult(obj0Wrap, obj1Wrap), m_resultCallback(resultCallback) {}
...
btSingleContactCallbackCustom::btSingleContactCallbackCustom(btCollisionObject *collisionObject, btCollisionWorld *world, btCollisionWorld::ContactResultCallback &resultCallback) : m_collisionObject(collisionObject), m_world(world), m_resultCallback(resultCallback) {}
