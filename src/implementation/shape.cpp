...
pragma::physics::BtShape::BtShape(IEnvironment &env, const std::shared_ptr<btCollisionShape> &shape) : IShape {env}, m_shape {shape}, m_externalShape {nullptr} {}
pragma::physics::BtShape::BtShape(IEnvironment &env, btCollisionShape *shape, bool bOwns) : IShape {env}, m_shape(nullptr), m_externalShape(nullptr)
{
	if(bOwns == true)
...
pragma::physics::BtConvexShape::BtConvexShape(IEnvironment &env, const std::shared_ptr<btConvexShape> &shape) : IConvexShape {env}, BtShape {env, shape}, IShape {env} {}
btConvexShape &pragma::physics::BtConvexShape::GetBtConvexShape() { return static_cast<btConvexShape &>(GetBtShape()); }

void pragma::physics::BtConvexShape::SetLocalScaling(const Vector3 &scale) { GetBtConvexShape().setLocalScaling(btVector3(scale.x, scale.y, scale.z)); }

//////////////////////////////////

pragma::physics::BtConvexHullShape::BtConvexHullShape(IEnvironment &env, const std::shared_ptr<btConvexHullShape> &shape) : IShape {env}, IConvexShape {env}, IConvexHullShape {env}, BtConvexShape {env, shape} {}
btConvexHullShape &pragma::physics::BtConvexHullShape::GetBtConvexHullShape() { return static_cast<btConvexHullShape &>(GetBtShape()); }

void pragma::physics::BtConvexHullShape::AddPoint(const Vector3 &point) { GetBtConvexHullShape().addPoint(btVector3(point.x, point.y, point.z) * BtEnvironment::WORLD_SCALE); }
...
pragma::physics::BtTriangleShape::BtTriangleShape(IEnvironment &env, const std::shared_ptr<btBvhTriangleMeshShape> &shape) : ITriangleShape {env}, BtShape {env, shape}, IShape {env} {}
...
void pragma::physics::BtTriangleShape::DoBuild(const std::vector<SurfaceMaterial> *materials)
{
	...
	if(materials == nullptr) {
		...
		m_triangleArray = std::make_unique<btTriangleIndexVertexMaterialArray>(m_triangles.size() / 3, reinterpret_cast<int32_t *>(m_triangles.data()), sizeof(m_triangles[0]) * 3, btVerts.size(), reinterpret_cast<btScalar *>(btVerts.data()), sizeof(btVerts[0]), materials->size(),
		  reinterpret_cast<unsigned char *>(const_cast<SurfaceMaterial *>(materials->data())), sizeof(SurfaceMaterial), m_faceMaterials.data(), sizeof(m_faceMaterials[0]));
...
void pragma::physics::BtTriangleShape::CalculateLocalInertia(float, Vector3 *localInertia) const { *localInertia = Vector3(0.f, 0.f, 0.f); }

//////////////////////////////////

pragma::physics::BtCompoundShape::BtCompoundShape(IEnvironment &env) : IShape {env}, ICompoundShape {env}, BtShape {env, nullptr} {}
