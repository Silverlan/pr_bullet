...
void pragma::physics::BtVisualDebugger::drawLine(const btVector3 &from, const btVector3 &to, const btVector3 &color) { drawLine(from, to, color, color); }
...
{
	DrawLine(Vector3(from.x() / BtEnvironment::WORLD_SCALE, from.y() / BtEnvironment::WORLD_SCALE, from.z() / BtEnvironment::WORLD_SCALE), Vector3(to.x() / BtEnvironment::WORLD_SCALE, to.y() / BtEnvironment::WORLD_SCALE, to.z() / BtEnvironment::WORLD_SCALE),
	  Color(static_cast<int16_t>(fromColor.x() * 255.f), static_cast<int16_t>(fromColor.y() * 255.f), static_cast<int16_t>(fromColor.z() * 255.f)), Color(static_cast<int16_t>(toColor.x() * 255.f), static_cast<int16_t>(toColor.y() * 255.f), static_cast<int16_t>(toColor.z() * 255.f)));
}
