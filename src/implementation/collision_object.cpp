...
SimpleMotionState::SimpleMotionState(pragma::physics::BtCollisionObject &o) : collisionObject {o} {}
...
pragma::physics::BtCollisionObject::BtCollisionObject(IEnvironment &env, std::unique_ptr<btCollisionObject> o, IShape &shape) : ICollisionObject {env, shape}, m_collisionObject {std::move(o)}
{
	GetInternalObject().setUserPointer(static_cast<ICollisionObject *>(this));
	ApplyCollisionShape(&shape);
}

void pragma::physics::BtCollisionObject::InitializeLuaHandle(const pragma::util::TWeakSharedHandle<IBase> &handle)
{
...
{
	if(pragma::math::is_flag_set(m_btStateFlags, BtStateFlags::WorldObjectRemoved))
		return;
	if(awake) {
		m_lastTickAwake = GetBtEnv().GetCurrentSimulationStepIndex();
		if(pragma::math::is_flag_set(m_btStateFlags, BtStateFlags::Awake))
			return;
		pragma::math::set_flag(m_btStateFlags, BtStateFlags::Awake);
	...
	if(!pragma::math::is_flag_set(m_btStateFlags, BtStateFlags::Awake))
		return;
	pragma::math::set_flag(m_btStateFlags, BtStateFlags::Awake, false);
...
void pragma::physics::BtCollisionObject::DoSpawn()
{
	auto *broadphase = m_collisionObject->getBroadphaseHandle();
	if(broadphase == nullptr)
		return;
	broadphase->m_collisionFilterMask = static_cast<int16_t>(pragma::math::to_integral(m_collisionFilterMask));
	broadphase->m_collisionFilterGroup = static_cast<int16_t>(pragma::math::to_integral(m_collisionFilterGroup));
	UpdateCCD();
}

void pragma::physics::BtCollisionObject::PutToSleep() { SetActivationState(ActivationState::WaitForDeactivation); }

void pragma::physics::BtCollisionObject::SetSimulationEnabled(bool b)
{
	if(b == IsSimulationEnabled())
		return;
	m_simState.simulationEnabled = b;
	if(b == false) {
		...
		m_simState.lastCollisionGroup = GetCollisionFilterGroup();
		SetCollisionFilterGroup(CollisionMask::NoCollision);
	}
	else {
		m_collisionObject->setCollisionFlags(m_collisionObject->getCollisionFlags() & ~btCollisionObject::CF_NO_CONTACT_RESPONSE);
		SetCollisionFilterGroup(m_simState.lastCollisionGroup);
		m_collisionObject->forceActivationState(ACTIVE_TAG);
		m_collisionObject->activate();
	}
}
bool pragma::physics::BtCollisionObject::IsSimulationEnabled() const { return m_simState.simulationEnabled; }
...
{
	pragma::math::set_flag(m_btStateFlags, BtStateFlags::CcdEnabled, b);
	UpdateCCD();
}
...
void pragma::physics::BtCollisionObject::UpdateCCD()
{
	...
	if(pragma::math::is_flag_set(m_btStateFlags, BtStateFlags::CcdEnabled) == false || m_collisionFilterGroup != CollisionMask::Default || m_collisionFilterMask != CollisionMask::Default) {
...
pragma::math::Transform pragma::physics::BtCollisionObject::GetBaseTransform() { return BtEnvironment::CreateTransform(m_collisionObject->getWorldTransform()); }
void pragma::physics::BtCollisionObject::SetBaseTransform(const pragma::math::Transform &t) { m_collisionObject->setWorldTransform(BtEnvironment::CreateBtTransform(t)); }

pragma::math::Transform pragma::physics::BtCollisionObject::GetWorldTransform() { return BtEnvironment::CreateTransform(m_collisionObject->getWorldTransform()) * m_localPoseInv; }

void pragma::physics::BtCollisionObject::SetWorldTransform(const pragma::math::Transform &t) { m_collisionObject->setWorldTransform(BtEnvironment::CreateBtTransform(t * m_localPose)); }
...
void pragma::physics::BtCollisionObject::DoSetCollisionFilterGroup(CollisionMask group)
{
	...
	if(broadphase == nullptr)
		return;
	broadphase->m_collisionFilterGroup = static_cast<int16_t>(pragma::math::to_integral(group));
}
...
void pragma::physics::BtCollisionObject::DoSetCollisionFilterMask(CollisionMask mask)
{
	...
	if(broadphase == nullptr)
		return;
	broadphase->m_collisionFilterMask = static_cast<int16_t>(pragma::math::to_integral(mask));
}
...
pragma::physics::BtCollisionObject::ActivationState pragma::physics::BtCollisionObject::GetActivationState() const
{
	...
	static_assert(pragma::math::to_integral(ActivationState::Count) == 5);
	return state;
}
...
void pragma::physics::BtCollisionObject::SetContactProcessingThreshold(float threshold) { m_collisionObject->setContactProcessingThreshold(threshold); }
...
void pragma::physics::BtCollisionObject::OnAddWorldObject() { pragma::math::set_flag(m_btStateFlags, BtStateFlags::WorldObjectRemoved, false); }
void pragma::physics::BtCollisionObject::OnRemoveWorldObject()
{
	SetAwake(false);
	pragma::math::set_flag(m_btStateFlags, BtStateFlags::WorldObjectRemoved);
}
...
void pragma::physics::BtCollisionObject::SetTrigger(bool bTrigger)
{
	if(bTrigger)
		m_collisionObject->setCollisionFlags(m_collisionObject->getCollisionFlags() | btCollisionObject::CF_NO_CONTACT_RESPONSE);
	else
...
bool pragma::physics::BtCollisionObject::IsTrigger() const
{
	int flags = m_collisionObject->getCollisionFlags();
	return (flags & btCollisionObject::CF_NO_CONTACT_RESPONSE) != 0 ? true : false;
}

void pragma::physics::BtCollisionObject::TransformLocalPose(const pragma::math::Transform &t) { m_localPose = t * m_localPose; }

void pragma::physics::BtCollisionObject::SetSleepReportEnabled(bool reportEnabled)
{
	// TODO
}
bool pragma::physics::BtCollisionObject::IsSleepReportEnabled() const
{
	// TODO
	return false;
}
...
pragma::physics::BtRigidBody::BtRigidBody(IEnvironment &env, std::unique_ptr<btRigidBody> body, IShape &shape) : ICollisionObject {env, shape}, BtCollisionObject {env, std::move(body), shape}, IRigidBody {env, shape}
{
...
static std::unique_ptr<btRigidBody> create_rigid_body(pragma::physics::BtShape &shape)
{
	Vector3 localInertia;
	shape.CalculateLocalInertia(shape.GetMass(), &localInertia);
	return std::make_unique<btRigidBody>(shape.GetMass(), nullptr, &shape.GetBtShape(), btVector3(localInertia.x, localInertia.y, localInertia.z) * pragma::physics::BtEnvironment::WORLD_SCALE);
}
pragma::physics::BtRigidBody::BtRigidBody(IEnvironment &env, BtShape &shape) : BtRigidBody(env, create_rigid_body(shape), shape) {}
...
Vector3 pragma::physics::BtRigidBody::GetTotalForce() const
{
	auto &body = GetInternalObject();
	auto force = body.getTotalForce() / BtEnvironment::WORLD_SCALE;
	return Vector3(force.x(), force.y(), force.z());
}
Vector3 pragma::physics::BtRigidBody::GetTotalTorque() const
{
	auto &body = GetInternalObject();
...
void pragma::physics::BtRigidBody::DoAddWorldObject()
{
	...
	OnAddWorldObject();
	auto *world = GetBtEnv().GetWorld();
	world->addRigidBody(&GetInternalObject(), pragma::math::to_integral(m_collisionFilterGroup), pragma::math::to_integral(m_collisionFilterMask));
}
...
void pragma::physics::BtRigidBody::SetMassAndUpdateInertia(float mass)
{
	// TODO
}
Vector3 pragma::physics::BtRigidBody::GetCenterOfMass() const
{
	// TODO
	return {};
}
void pragma::physics::BtRigidBody::SetCenterOfMassOffset(const Vector3 &offset)
{
	// TODO
}
Vector3 pragma::physics::BtRigidBody::GetCenterOfMassOffset() const
{
	// TODO
	return {};
}
...
void pragma::physics::BtRigidBody::ApplyForce(const Vector3 &force, bool autoWake)
{
	if(IsStatic())
		return;
	if(autoWake)
...
void pragma::physics::BtRigidBody::ApplyForce(const Vector3 &force, const Vector3 &relPos, bool autoWake)
{
	if(IsStatic())
		return;
	if(autoWake)
...
void pragma::physics::BtRigidBody::ApplyImpulse(const Vector3 &impulse, bool autoWake)
{
	if(IsStatic())
		return;
	if(autoWake)
...
void pragma::physics::BtRigidBody::ApplyImpulse(const Vector3 &impulse, const Vector3 &relPos, bool autoWake)
{
	if(IsStatic())
		return;
	if(autoWake)
...
void pragma::physics::BtRigidBody::ApplyTorque(const Vector3 &torque, bool autoWake)
{
	if(IsStatic())
		return;
	GetInternalObject().activate(true);
...
void pragma::physics::BtRigidBody::ApplyTorqueImpulse(const Vector3 &torque, bool autoWake)
{
	if(IsStatic())
		return;
	if(autoWake)
...
bool pragma::physics::BtRigidBody::IsKinematic() const { return (GetCollisionFlags() & btCollisionObject::CF_KINEMATIC_OBJECT) != 0; }
...
void pragma::physics::BtRigidBody::SetLinearVelocity(const Vector3 &vel, bool autoWake)
{
	if(IsStatic() == true) {
...
void pragma::physics::BtRigidBody::SetAngularVelocity(const Vector3 &vel, bool autoWake)
{
	if(IsStatic() == true) {
...
void pragma::physics::BtRigidBody::PreSimulate()
{
	...
	if(IsKinematic()) {
		...
		EulerAngles ang {static_cast<float>(pragma::math::rad_to_deg(angVel.x)), static_cast<float>(pragma::math::rad_to_deg(angVel.y)), static_cast<float>(pragma::math::rad_to_deg(angVel.z))};
...
void pragma::physics::BtSoftBody::DoAddWorldObject()
{
	...
	OnAddWorldObject();
	auto *world = GetBtEnv().GetWorld();
	world->addSoftBody(&GetInternalObject(), pragma::math::to_integral(m_collisionFilterGroup), pragma::math::to_integral(m_collisionFilterMask));
#endif
}
...
void pragma::physics::BtSoftBody::SetMass(int32_t node, float mass)
{
	GetInternalObject().setMass(node, mass);
	UpdateTotalMass();
}
void pragma::physics::BtSoftBody::SetMass(float mass)
{
	GetInternalObject().setTotalMass(mass);
	UpdateTotalMass();
}
...
void pragma::physics::BtSoftBody::SetWorldTransform(const pragma::math::Transform &t)
{
...
void pragma::physics::BtSoftBody::AddVelocity(const Vector3 &vel) { GetInternalObject().addVelocity(uvec::create_bt(vel) * BtEnvironment::WORLD_SCALE); }
...
pragma::physics::BtGhostObject::BtGhostObject(IEnvironment &env, std::unique_ptr<btPairCachingGhostObject> o, IShape &shape) : ICollisionObject {env, shape}, BtCollisionObject(env, std::move(o), shape), IGhostObject {env, shape} {}
...
void pragma::physics::BtGhostObject::DoAddWorldObject()
{
	...
	OnAddWorldObject();
	auto *world = GetBtEnv().GetWorld();
	world->addCollisionObject(m_collisionObject.get(), pragma::math::to_integral(m_collisionFilterGroup), pragma::math::to_integral(m_collisionFilterMask)); //,btBroadphaseProxy::CharacterFilter,btBroadphaseProxy::StaticFilter | btBroadphaseProxy::DefaultFilter);
}
