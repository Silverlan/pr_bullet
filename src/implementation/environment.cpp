...
namespace pragma::physics {
	class BtDebugDrawer : public btIDebugDraw {
	  public:
		BtDebugDrawer(pragma::physics::BtEnvironment &env, pragma::physics::IVisualDebugger &debugger) : m_env {env}, m_debugger {debugger} {}
		...
		{
			m_debugger.DrawLine(pragma::physics::BtEnvironment::ToPragmaPosition(from), pragma::physics::BtEnvironment::ToPragmaPosition(to), pragma::physics::BtEnvironment::ToPragmaColor(fromColor), pragma::physics::BtEnvironment::ToPragmaColor(toColor));
		}
		virtual void drawLine(const btVector3 &from, const btVector3 &to, const btVector3 &color) override
		{
			m_debugger.DrawLine(pragma::physics::BtEnvironment::ToPragmaPosition(from), pragma::physics::BtEnvironment::ToPragmaPosition(to), pragma::physics::BtEnvironment::ToPragmaColor(color));
		}
		...
		virtual void drawTriangle(const btVector3 &a, const btVector3 &b, const btVector3 &c, const btVector3 &color, btScalar alpha) override
		{
			...
			col.a = alpha * 255.f;
			m_debugger.DrawTriangle(pragma::physics::BtEnvironment::ToPragmaPosition(a), pragma::physics::BtEnvironment::ToPragmaPosition(b), pragma::physics::BtEnvironment::ToPragmaPosition(c), col, col, col);
		}
		...
		virtual void reportErrorWarning(const char *warningString) override { m_debugger.ReportErrorWarning(warningString); }
		virtual void draw3dText(const btVector3 &location, const char *textString) override { m_debugger.DrawText(textString, pragma::physics::BtEnvironment::ToPragmaPosition(location), colors::White, 1.f); }
...
#pragma optimize("", off)
class PhysBulletWorld : public pragma::physics::btWorldType {
  public:
...
void pragma::physics::BtEnvironment::OnVisualDebuggerChanged(pragma::physics::IVisualDebugger *debugger)
{
	...
	m_btDebugDrawer->setDebugMode(btIDebugDraw::DebugDrawModes::DBG_DrawWireframe | btIDebugDraw::DebugDrawModes::DBG_DrawAabb | btIDebugDraw::DebugDrawModes::DBG_DrawConstraints);
}
...
std::shared_ptr<pragma::physics::IConvexShape> pragma::physics::BtEnvironment::CreateCapsuleShape(float halfWidth, float halfHeight, const IMaterial &mat)
{
	...
	return CreateSharedPtr<BtConvexShape>(*this, std::make_shared<btCapsuleShape>(halfWidth * pragma::physics::BtEnvironment::WORLD_SCALE, height * pragma::physics::BtEnvironment::WORLD_SCALE));
}
...
{
	return CreateSharedPtr<BtConvexShape>(*this, std::make_shared<btBoxShape>(btVector3(halfExtents.x, halfExtents.y, halfExtents.z) * pragma::physics::BtEnvironment::WORLD_SCALE));
}
...
{
	return CreateSharedPtr<BtConvexShape>(*this, std::make_shared<btCylinderShape>(btVector3(radius, height, radius) * pragma::physics::BtEnvironment::WORLD_SCALE));
}
std::shared_ptr<pragma::physics::IConvexShape> pragma::physics::BtEnvironment::CreateSphereShape(float radius, const IMaterial &mat) { return CreateSharedPtr<BtConvexShape>(*this, std::make_shared<btSphereShape>(radius * pragma::physics::BtEnvironment::WORLD_SCALE)); }
...
std::shared_ptr<pragma::physics::IConvexHullShape> pragma::physics::BtEnvironment::CreateConvexHullShape(const IMaterial &mat) { return CreateSharedPtr<BtConvexHullShape>(*this, std::make_shared<btConvexHullShape>()); }
std::shared_ptr<pragma::physics::ITriangleShape> pragma::physics::BtEnvironment::CreateTriangleShape(const IMaterial &mat) { return CreateSharedPtr<BtTriangleShape>(*this); }
...
namespace pragma::physics {
	...
	class PhysRayResultCallback : public TBase {
	  public:
		PhysRayResultCallback(pragma::physics::IRayCastFilterCallback *filter, const btVector3 &rayFromWorld, const btVector3 &rayToWorld) : m_filter {filter}, TBase {rayFromWorld, rayToWorld} {}
			...
			if(colObj != nullptr) {
				...
				if(physObj != nullptr) {
					...
					if(pPhysComponent && pPhysComponent->IsRayResultCallbackEnabled() == true) {
						if(pPhysComponent->RayResultCallback(static_cast<CollisionMask>(TBase::m_collisionFilterGroup), static_cast<CollisionMask>(TBase::m_collisionFilterMask)) == false)
							return 0.0;
					}
					// TODO: Filter
				}
	...
	class PhysAllHitsRayResultCallback : public PhysRayResultCallback<btCollisionWorld::AllHitsRayResultCallback> {
	  public:
		using PhysRayResultCallback<btCollisionWorld::AllHitsRayResultCallback>::PhysRayResultCallback;
	};

	class PhysClosestRayResultCallback : public PhysRayResultCallback<btCollisionWorld::ClosestRayResultCallback> {
	  public:
